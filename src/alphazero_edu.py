...
def heuristic_evaluate(grid, player, cfg):
    """
    Heuristic evaluation function (replaces neural network).
...
def get_policy_value(grid, player, cfg):
    ...
    Simplified version without actual neural network.
    """
    ...
    for action in valid_actions:
        ...
        # Check blocking opponent win
            ...
            if check_win(opp_grid, opponent, cfg):
                ...
                action_scores[action] = heuristic_evaluate(new_grid, player, cfg)
    
    # Convert to probabilities (softmax-like)
    max_score = max(action_scores.values())
    exp_scores = {a: math.exp((s - max_score) / 100) for a, s in action_scores.items()}
    total = sum(exp_scores.values())
    policy = {a: s / total for a, s in exp_scores.items()}
    
    # Value estimation
    value = heuristic_evaluate(grid, player, cfg) / 10000.0
...
if __name__ == "__main__":
    ...
    try:
        ...
        winner = "my_agent (Player 1)" if result[0]['reward'] == 1 else "random (Player 2)"
